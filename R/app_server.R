...
app_server <- function(input, output,session) {
  ...
  mod_human_readable_report_server("human_readable_report_ui_1", input_data = read_out$data, submit = read_out$submit)
  
  # Generate the first page with contributors affiliation
  mod_contribs_affiliation_page_server("contribs_affiliation_page_ui_1", input_data = read_out$data, submit = read_out$submit)
